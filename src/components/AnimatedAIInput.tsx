  ...
  return <div className={`relative ${className}`}>
      <form onSubmit={handleSubmit} className="relative">
        <div className={`flex items-center overflow-hidden bg-black/60 backdrop-blur-md border border-white/10 rounded-xl transition-all ${isFocused ? "shadow-lg ring-1 ring-white/20 border-white/30" : ""}`}>
          ...
          <HoverCard>
            ...
            <HoverCardContent className="w-56 bg-black/90 backdrop-blur-lg border border-white/10 text-white p-0">
              <div className="p-2">
                ...
                {models.map(model => <button key={model.id} className={`w-full text-left px-3 py-2 rounded-md text-sm transition-colors ${selectedModel === model.id ? 'bg-white/10 text-white' : 'hover:bg-white/5 text-white/80'} ${model.requiresPlan && !isPaidUser ? 'opacity-50' : ''}`} onClick={() => handleModelSelect(model.id)}>
          ...
          {showAttachButton && <button type="button" onClick={handleAttachClick} className="flex-shrink-0 ml-2 p-2 text-white/60 hover:text-white transition-colors">
              <Paperclip className="h-5 w-5" />
            </button>}
          
          <textarea ref={textAreaRef} value={value} onChange={e => onChange(e.target.value)} onKeyDown={handleKeyDown} placeholder={placeholder} onFocus={() => setIsFocused(true)} onBlur={() => setIsFocused(false)} disabled={disabled} className="w-full bg-transparent border-0 resize-none p-4 text-white placeholder:text-white/50 focus:ring-0 focus:outline-none transition-colors" rows={1} style={{
          minHeight: "40px",
          maxHeight: "200px"
        }} />
          
          <Button type="submit" disabled={!value.trim() || disabled || isLoading} className={`m-2 rounded-lg flex-shrink-0 p-0 w-10 h-10 items-center justify-center transition-all duration-300 ${!value.trim() || disabled ? "bg-white/10 text-white/50" : "bg-white text-black"}`}>
            {isLoading ? <div className="w-4 h-4 border-2 border-black/20 border-t-black rounded-full animate-spin" /> : <Send className={`h-4 w-4 ${value.trim() && !disabled ? "" : "opacity-50"}`} />}
          </Button>
        </div>
      </form>
    </div>;
